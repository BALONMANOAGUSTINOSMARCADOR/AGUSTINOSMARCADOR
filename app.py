...
if st.button("Guardar partido en GitHub"):
    ...
    partido = {
        ...
        "part": match.get("part", 1),
        "elapsed_before_pause": match["elapsed_before_pause"],
        "started_at": None  # Lo guardamos como finalizado
    }
...
if st.session_state.view == "buscar_partidos":
    ...
    if results:
        ...
        for i, p in enumerate(results):
            ...
            if st.button(f"Cargar partido {p.get('rival')} ({i})"):

                # Restaurar TODO el estado
                st.session_state.match = p

                # El partido cargado no está en marcha
                st.session_state.match["started_at"] = None
